...
export const Dialog: React.FC<DialogProps> = ({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[200] flex items-center justify-center px-4 pointer-events-auto">
      ...
      {/* Dialog Box */}
      <div
        className={`relative w-full max-w-[320px] bg-background border border-border rounded-3xl p-6 shadow-xl transition-all duration-300 transform ${
        ...
        <div className="flex flex-col gap-3">
          ...
          {(secondaryButtonText && secondaryButtonOnClick) && (
            <button
              ...
              className="w-full h-[44px] rounded-full bg-card flex items-center justify-center text-white font-semibold hover:bg-secondary active:scale-95 transition-all"
            >
              {secondaryButtonText}
            </button>
          )}
