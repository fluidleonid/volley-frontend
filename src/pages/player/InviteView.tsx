...
export const InviteView: React.FC<InviteViewProps> = ({ onClose }) => {
  ...
  return (
    <div className="h-full min-h-screen flex flex-col bg-background text-white px-4 max-w-[480px] mx-auto select-none">
      ...
      <div className="flex-1 flex flex-col space-y-4 pb-[110px] overflow-hidden">
        ...
        <div className="relative w-full h-[400px] rounded-[40px] bg-card p-[20px] shadow-2xl border-0 overflow-hidden flex flex-col justify-between shrink-0">
          ...
          <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 border-b border-dashed border-border/60 pointer-events-none z-10" />

          {/* TOP BOX: Team A Section (Entire box area is clickable for intuitive UX) */}
          <div
            onClick={() => setActivePicker('teamA')}
            className="relative w-full h-[160px] rounded-[20px] bg-brand-surfaceElevated hover:bg-[#2A2A2C] transition-all active:scale-[0.99] flex items-center justify-center cursor-pointer select-none"
          >
            <div className="flex items-center -space-x-2">
              {teamAPlayers.map((player, idx) => (
                <Avatar
                  ...
                  size="lg"
                  hasBorder={false}
                  className={`relative ring-2 ring-brand-surfaceElevated z-[${10 - idx}]`}
                />
              ))}
        ...
        <div className="text-center shrink-0">
          <p className="font-sans text-sm text-muted-foreground transition-colors duration-200">
            {getHelperSubtitle()}
          </p>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 inset-x-0 pb-[48px] pt-4 px-4 max-w-[480px] mx-auto z-20 bg-background/80 backdrop-blur-md">
        <button
          ...
          className={`w-full h-[52px] rounded-full bg-primary text-primary-foreground font-sans text-base font-bold transition-all ${isReadyToInvite
            ? 'shadow-lg shadow-primary/20 hover:bg-primary/90 active:scale-95 cursor-pointer opacity-100'
